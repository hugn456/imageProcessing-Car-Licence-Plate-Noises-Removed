...
def computeConnectedComponentLabeling(pixel_array, image_width, image_height):
    ...
    for i in range (image_height):
        for j in range ( image_width):
            if new_array[i][j] !=0:
                if new_array[i][j] not in dictt.keys():
                    dictt[new_array[i][j] ]=1
                else:
                    dictt[new_array[i][j] ]+=1
                    
    return (new_array,dictt) 

# compute minmax x,y
def computeMinMaxXY(ccimg, ccsizedict, image_width, image_height):
...
# compute median filter to remove  salt and pepper noise 
def computeMedian5x5ZeroPadding(pixel_array, image_width, image_height):
    array=createInitializedGreyscalePixelArray(image_width, image_height)
    for x in range (image_height):
        for y in range(image_width):
            sortList=[]
            for xx in [-2,-1,0,1,2]:
                for yy in [-2,-1,0,1,2]:
                    if (x+xx >=0) and (y+yy >=0) and (x+xx <image_height) and (y+yy <image_width):
                        sortList.append(pixel_array[x+xx][y+yy])
                    else:
                        sortList.append(0)
            sortList.sort()
            
            array[x][y]=sortList[12]
    return array

# compute mean filter to remove white noise
        ...
        for y in range(image_width):
            if (x==0) or (y==0) or (x==image_height-1) or (y==image_width-1):
                ...
                for xx in [-2,-1,0,1,2]:
                    for yy in [-2,-1,0,1,2]:
                        outputPixel+=pixel_array[x+xx][y+yy]
                outputPixel=outputPixel/25.0
                array[x][y]=outputPixel
    return array
...
def main():
    ...
    input_filename = "numberplate1dot.png"
    ...
    threshold_value=computeAdaptiveThreshold(contrast_stretched_pixel_array, image_width, image_height) +90  
    threshold_value=150
    ...
    (min_x, max_x, min_y, max_y)= computeMinMaxXY(ccimg, ccsizedict, image_width, image_height)
    ...
    axs1[2, 1].add_patch(rect2) 
     ...
     # noise remove---------------------------------------------------------------------------
    median_noise_remove=computeMedian5x5ZeroPadding(stretched_greyscale_pixel_array, image_width, image_height)
    stretched_median_noise_remove=scaleTo0And255AndQuantize(median_noise_remove, image_width, image_height)
    
    #mean_noise_remove= computeBoxAveraging3x3(stretched_greyscale_pixel_array, image_width, image_height)
    #stretched_mean_noise_remove=scaleTo0And255AndQuantize(mean_noise_remove, image_width, image_height)

    ## Assume that the image has salt and pepper noise 
    # Contrast Stretching
    median_smoothed_image = computeStandardDeviationImage5x5(stretched_median_noise_remove, image_width, image_height)
    median_contrast_stretched_pixel_array= scaleTo0And255AndQuantize(median_smoothed_image, image_width, image_height)
    
    # thresholding operation
    median_threshold_value=computeAdaptiveThreshold(median_contrast_stretched_pixel_array, image_width, image_height)  
    median_threshold_value=100
    median_thresholded = computeThresholdGE(median_contrast_stretched_pixel_array, median_threshold_value, image_width, image_height) 
    median_dilated_eroded_image=list(median_thresholded)
   
    #perform dilations and erosions
    # several dilations
    for i in range(4):
        median_dilated_eroded_image =computeDilation8Nbh3x3FlatSE(median_dilated_eroded_image , image_width, image_height)

    # followed by several erosions
    for j in range(4):
        median_dilated_eroded_image=computeErosion8Nbh3x3FlatSE(median_dilated_eroded_image, image_width, image_height)
    
    # perform connected component analysis
    (ccimg,ccsizedict) = computeConnectedComponentLabeling(median_dilated_eroded_image ,image_width,image_height)
    (min_xx, max_xx, min_yy, max_yy)= computeMinMaxXY(ccimg, ccsizedict, image_width, image_height)
    
    
    axs1[0, 2].set_title('Original')
    axs1[0, 2].imshow(imagee)
    axs1[0, 3].set_title('Gray Scale')
    axs1[0, 3].imshow(stretched_greyscale_pixel_array, cmap='gray')
    axs1[0, 4].set_title('Noise-removed Gray Scale')
    axs1[0, 4].imshow(stretched_median_noise_remove, cmap='gray')
    axs1[1, 2].set_title('Noise-removed Contrast Stretching')
    axs1[1, 2].imshow(median_contrast_stretched_pixel_array, cmap='gray')
    axs1[1, 3].set_title('Noise-removed Thresholding')
    axs1[1, 3].imshow(median_thresholded, cmap='gray')
    axs1[2, 2].set_title('Noise-removed Dilitations and Erosions')
    axs1[2, 2].imshow(median_dilated_eroded_image, cmap='gray')
    axs1[2, 3].set_title('Noise-removed Final Image')
    axs1[2, 3].imshow(imagee)
    
    rect3=Rectangle((min_xx, min_yy), max_xx - min_xx, max_yy - min_yy, linewidth=3,
                     edgecolor='g', facecolor='none')
    rect4=Rectangle((min_xx, min_yy), max_xx - min_xx, max_yy - min_yy, linewidth=3,
                     edgecolor='g', facecolor='none')
    axs1[2, 2].add_patch(rect3)
    axs1[2, 3].add_patch(rect4)
